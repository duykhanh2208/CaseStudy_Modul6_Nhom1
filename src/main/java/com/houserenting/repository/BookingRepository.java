...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query(value = "select booking.* from booking join house on booking.house_id = house.id where house.owner_id = ? and (booking.status='Đang ở' or booking.status='Chờ nhận phòng')",nativeQuery = true)
    List<Booking> ShowListBookingOfTheOwner(@Param("id") Long id);
    @Query(value = "select booking.* from booking join house on booking.house_id = house.id where house.owner_id = ? and (booking.status!='Đang ở' and booking.status!='Chờ nhận phòng') order by booking.create_at desc",nativeQuery = true)
    List<Booking> ShowBookingHistoryForOwner(@Param("id") Long id);
